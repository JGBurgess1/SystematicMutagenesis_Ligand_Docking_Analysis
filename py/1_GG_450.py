...
#parser.add_argument("-pos","--position", required=True, choices = ['0', '1', '2', '3', '4', '5', '6'], help="position within the queue. Allowed values 0, 1, 2, 3, 4")
...
for residue in complex_structure.residue:
    ...
    """
    if args.position == '0' and int(residue.resnum) < 77:
        continue
    if args.position == '1' and int(residue.resnum) < 158:
        continue
    if args.position == '2' and int(residue.resnum) < 238:
        continue
    if args.position == '3' and int(residue.resnum) < 318:
        continue
    if args.position == '4' and int(residue.resnum) < 398:
        continue
    if args.position == '5' and int(residue.resnum) < 478:
        continue
    if args.position == '6' and int(residue.resnum) < 558:
        continue
    """
    if int(residue.resnum) != 499:
        continue
    ...
    ####### > glide SP < #######
        ...
        glide_SP_inp_file_name = f"{modified_file_name[:-4]}_glide_XP.inp"
